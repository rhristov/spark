...
class BlockManagerSuite extends SparkFunSuite with Matchers with BeforeAndAfterEach
  ...
  test("block compression") {
    try {
      ...
      store.putSingle(ShuffleBlockId(0, 0, 0, 1), new Array[Byte](1000), StorageLevel.MEMORY_ONLY_SER)
      assert(store.memoryStore.getSize(ShuffleBlockId(0, 0, 0, 1)) <= 100,
      ...
      store.putSingle(ShuffleBlockId(0, 0, 0, 1), new Array[Byte](10000), StorageLevel.MEMORY_ONLY_SER)
      assert(store.memoryStore.getSize(ShuffleBlockId(0, 0, 0, 1)) >= 10000,
