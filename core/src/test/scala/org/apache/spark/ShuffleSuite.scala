...
abstract class ShuffleSuite extends SparkFunSuite with Matchers with LocalSparkContext {
  ...
  test("[SPARK-4085] rerun map stage if reduce stage cannot find its local shuffle file") {
    ...
    val hashFile = sc.env.blockManager.diskBlockManager.getFile(new ShuffleBlockId(0, 0, 0, 1))
    val sortFile = sc.env.blockManager.diskBlockManager.getFile(new ShuffleDataBlockId(0, 0))
